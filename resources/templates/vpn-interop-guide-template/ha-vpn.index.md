---
title: Google Cloud High Availability VPN Interop Guide for [PRODUCT]
...
# Using Cloud HA VPN with <vendor-name><product-name>

Learn how to build site-to-site IPsec VPNs between
[Cloud HA VPN](https://cloud.google.com/vpn/docs/)on Google Cloud Platform (GCP) and
...
- [Google Cloud HA VPN Interop Guide](#google-cloud-vpn-interop-guide)
...
[Cloud HA VPN service](https://cloud.google.com/vpn/docs) on GCP.
...
This section covers how to configure Cloud HA VPN. Note that Cloud HA VPN only supports
dynamic routing.
...
export CUST_GW_EXT_IP_1=[CUST_GW_EXT_IP_1]
export CUST_GW_EXT_IP_2=[CUST_GW_EXT_IP_2]
...
1. Create a GCP VPN gateway in the region you are using.
    ...
    ```
    gcloud compute vpn-gateways create $VPN_GATEWAY_1 \
        --project $PROJECT_NAME \
        --network $VPC_NETWORK_NAME \
        --region $REGION
    ```
...
1. Create 2 VPN tunnels, one on each interface that points to the external
IP addresses of your on-premises VPN gateways. Note the following:
    ...
    gcloud compute vpn-tunnels create $VPN_TUNNEL_1 \
        ...
        --region $REGION \
        --ike-version 2 \
        --vpn-gateway $VPN_GATEWAY_1 \
        --router $CLOUD_ROUTER_NAME \
        --peer-address $CUST_GW_EXT_IP_1 \
